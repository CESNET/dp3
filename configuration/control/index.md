...
The `Control` module serves the [`/control/...`](../../api/#control) endpoints. This is used to execute actions that are not part of the normal operation of the application, at a time when the user decides to do so. Useful for management of the application, especially when there are changes made to the configuration.
...
1. Re-runs the `on_entity_creation` callback for selected `etype`
1. Re-runs the `load_config` for selected module and will refresh the values derived by the module
