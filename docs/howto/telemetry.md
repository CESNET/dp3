...
## 5. Check secondary modules in worker logs

DP³ does not expose detailed secondary-module execution diagnostics through the API. Inspect the worker logs instead:
...
Secondary-module callback registration is logged during application startup. If a callback appears not to run, restart the affected workers and search the startup logs for the module class name. This confirms whether its hooks were registered and also exposes import or configuration errors.
...
The `te` group contains task-processing and error counters. The `tasks_by_src` group contains one counter per observed datapoint source. Groups and intervals are application-configurable; check `event_logging.yml` for the available values.

This command uses the Redis connection from the selected DP³ configuration directory and must run from a host that can reach that Redis instance. Current counts may lag behind workers by the configured EventCountLogger synchronization interval. Last counts cover the most recently completed interval.
