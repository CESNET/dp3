...
class DP3ModuleTestCase(ModuleAssertions, unittest.TestCase, Generic[ModuleT]):
    ...
    def make_registrar(self) -> TestCallbackRegistrar:
        update_batch_period = self.config.get("updater.update_batch_period", None)
        return TestCallbackRegistrar(self.model_spec, update_batch_period=update_batch_period)
