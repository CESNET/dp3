...
1. deploy the tested configuration and code to the server
1. run the long-lived API and worker processes under a process manager
1. use logs, status checks, and telemetry to watch the deployment after changes

If you do not already have a working local development setup, start with [Get started with local DP³ app development](https://cesnet.github.io/dp3/howto/get-started/index.md).
...
We recommend placing both the application and its virtual environment outside the home directory, for example under `/opt/<APP_NAME>`. This keeps permissions and service management simpler.
...
1. The `deploy` extras include the `gunicorn` server used by the generated supervisor configuration.
...
1. Enables the RabbitMQ [management interface](https://www.rabbitmq.com/management.html), which is useful both during setup and later troubleshooting.
...
The generated supervisor configuration lives under `/etc/<APP_NAME>` and the process logs are written under `/var/log/<APP_NAME>`. Worker processes exit with code `1` on internal failures so supervisor can restart them. Intentional `SIGINT`/`SIGTERM` shutdowns exit with code `0`, and configuration errors exit with code `2` so supervisor leaves the worker stopped until the configuration is fixed. For more on supervisor itself, see the [supervisorctl documentation](http://supervisord.org/running.html#running-supervisorctl).
...
You can also enable shell completion for the wrapper or for `dp3` itself. The completion is registered for the executable name seen by the shell:
...
1. update the code and configuration on the server
1. validate the configuration
1. restart only the processes that need the new state
1. watch status and logs after the restart
...
1. Validate the exact configuration directory used in production before restarting anything.
1. For smaller changes, restart only the affected process group if you prefer.
...
- **Only module YAML changes**: if your module uses `load_config`, you can use [`refresh_module_config`](https://cesnet.github.io/dp3/configuration/control/#refresh_module_config) instead of a full worker restart.
- **Changed worker count**: rerun `dp3-script rmq_reconfigure.sh <APP_NAME> <NUM_WORKERS>` before restarting workers - [read more details here](https://cesnet.github.io/dp3/configuration/processing_core/#worker-processes).
...
- [Get started with local DP³ app development](https://cesnet.github.io/dp3/howto/get-started/index.md)
- [Configuration overview](https://cesnet.github.io/dp3/configuration/index.md)
- [API configuration](https://cesnet.github.io/dp3/configuration/api/index.md)
- [Database configuration](https://cesnet.github.io/dp3/configuration/database/index.md)
- [Event logging configuration](https://cesnet.github.io/dp3/configuration/event_logging/index.md)
- [Processing core configuration](https://cesnet.github.io/dp3/configuration/processing_core/index.md)
- [Control configuration](https://cesnet.github.io/dp3/configuration/control/index.md)
